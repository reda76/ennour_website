...
|---|---|---|---|---|
| 1 | **Adresse postale** | `ORG.adresse` | `'__ADRESSE_A_CONFIRMER__, 76600 Le Havre'` | Remplacer le marqueur par la rue et le numéro. La partie connue s’affiche en attendant. |
...
| 3 | **Horaires du secrétariat** | `MENTION_SECRETARIAT` | libellé d’attente | La FAQ y renvoie (« pendant les horaires d’ouverture ») sans pouvoir les donner. |
| 4 | **URL de la campagne HelloAsso** | `MOYENS_REGLEMENT` → `helloasso.url` | `null` | Le moyen est nommé mais pas cliquable. |
| 5 | **Sigle AME** | `ORG.association` | `null` | Le logo de l’association est affiché ; son nom complet est inconnu et n’est **pas** inventé. Le texte alternatif dit « association AME ». |
| 6 | **Dates du calendrier** | `CALENDRIER[].debut/.fin` | `provisoire: true` sur les 9 entrées | Calées sur la zone B, jamais confirmées. Retirer `provisoire` fait disparaître la réserve affichée en tête de section. |
...
| **Le cours d’arabe du mercredi** | `CRENEAUX` → `arabe-f` | Il ne relève d’aucun des trois pôles : la mosquée a précisé qu’il n’y a pas d’alphabétisation en semaine. `poles: []`, donc visible seulement dans la semaine non filtrée. À rattacher le jour où son rattachement est connu. |
| **Trois séances femmes en salle 2** | `CRENEAUX` → `fiqh-coran-f`, `arabe-f`, `coran-tafsir-f` | Elles figurent au classeur mais sur aucune formule de l’affiche. Affichées avec la mention « Nous consulter ». Sont-elles ouvertes aux inscriptions, et à quelles conditions ? |
